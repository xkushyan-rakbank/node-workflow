  ...
  if (field.hasOwnProperty('datalist') && field.datalist !== '') {
    let optionItems = field.datalist.map((item) =>
      <div className={rdDivClass} key={item.value} >
        <input className="form-check-input" key={item.value} type="radio" id={item.value} value={item.value} {...attr} {...(item.value===attr.value)?"checked":""}></input>
...
function RadioGroup({ id, indexes, reducerActionType, divClass }) {
  ...
  const handleRadioChange = (event) => {
    if (reducerActionType !== null && reducerActionType !== undefined) {
      ...
      let newState = { ...state };
      let newValue = jp.value(newState, newPath, selectedOption);
  ...
  const attr = {
    "name": newId,
    "onChange": field.hasOwnProperty('onchange') ? field.onchange : handleRadioChange,
